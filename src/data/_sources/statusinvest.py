...
def _fmt_value(value: str) -> float | str:
    ...
    if "K" in value:
        ...
        mult = 1
    cleaned_v = (
        cleaned_v.replace("K", "").replace("M", "").replace("B", "").replace("%", "")
    )
...
def details(ticker: str) -> dict:
        ...
        if tag:
            ...
            try:
                if value_s == "-":
                    return float("nan")
                value = (
                    float(value_s.replace(".", "").replace(",", ".").replace("%", ""))
                    * mult
                )
    ...
    if company_div:
        company_name = company_div.find(
            "span", class_="text-main-green-dark"
        ).text.strip()
