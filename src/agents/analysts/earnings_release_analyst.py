...
from src.agents.utils import pdf_bytes_to_text
...
def analyze(ticker: str, company_name: str, earnings_release_pdf_bytes: bytes) -> BaseAgentOutput:
    async def _analyze():
        text = pdf_bytes_to_text(earnings_release_pdf_bytes)
        r = await agent.run(f'Analise o seguinte earnings release da empresa {ticker} - {company_name}: {text}')
        return r.data
